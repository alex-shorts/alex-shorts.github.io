---
id: cornelius_bowerman_david
slug: cornelius-bowerman-david
name: Cornelius Bowerman
aka: son of David Bowerman
birth:
death:
parents: [david_bowerman, catherine_bartlett_bowerman]
parent_links:
  - id: david_bowerman
    confidence: Possible
  - id: catherine_bartlett_bowerman
    confidence: Possible
spouses: []
children: []
object_ids: []
confidence: Possible
verified: false
status: researching
sources:
  - label: "Bowerman family genealogy p. 16 — (VI) Cornelius, son of David × Catherine"
    url: "https://freepages.rootsweb.com/~saylormowbray/genealogy/bowerman1904.html"
---
## Summary

**Possible** sibling of Charlotte (Bowerman) Aylesworth (**C1196**). Named in David × Catherine child list. `verified: false`.
