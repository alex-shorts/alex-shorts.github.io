---
id: jenny_mamlock
slug: jenny-mamlock
name: Jenny Mamlock
aka: [Jennie Mamlock]
birth: "~1867"
death:
birth_place: Germany
death_place:
burial:
parents: [mayer_mamlock, rochelle_kahn]
spouses: []
children: []
object_ids: [FT-0260, FT-0305]
confidence: Probable
verified: false
status: researching
parent_links:
  - id: mayer_mamlock
    confidence: Probable
  - id: rochelle_kahn
    confidence: Probable
  - id: rosa_mamlock
    confidence: Probable
sources:
  - label: "ISTG Allemannia 9 Sep 1867 — Jenny Mamlock 0y6m with Rosa + sibs"
    url: "https://www.immigrantships.net/v14/1800v14/allemannia18670909_02.html"
  - label: "FS 1900 census index — Rachael Mamlock household includes Jennie Mamlock"
    url: "https://www.familysearch.org/en/search/record/results?q.givenName=Rachael&q.surname=Mamlock"
media: []
---

## Summary

**Probable** sister of **Mamie Mamlock** (collateral). Named on 1867 Allemannia passenger list and in 1900 household index under mother Rachael. Manifest FAN **Rosa Mamlock** (Probable ship mother — **C1522**). Soft only; do not climb ahead of Rochelle UP.

## Assertions

| Claim | Confidence | Source | Source URL | Notes |
| --- | --- | --- | --- | --- |
| Daughter of Mayer × Rochelle/Rachel; sib of Mamie | Probable | ISTG + FS 1900 index | https://www.immigrantships.net/v14/1800v14/allemannia18670909_02.html | Pack **C1073** |

## Sources

- [ISTG Allemannia 1867](https://www.immigrantships.net/v14/1800v14/allemannia18670909_02.html)
