---
id: daniel_knight
slug: daniel-knight
name: Daniel Knight
aka: []
birth: "1764-06-28"
death:
birth_place: Somersworth, Strafford County, New Hampshire, USA
death_place:
burial:
parents: [john_knight, lydia_roberts_knight]
spouses: []
children: []
object_ids: []
confidence: Probable
verified: false
status: researching
parent_links:
  - id: john_knight
    confidence: Probable
  - id: lydia_roberts_knight
    confidence: Probable
sources:
  - label: "WikiTree — Daniel Knight (Knight-5642)"
    url: "https://www.wikitree.com/wiki/Knight-5642"
  - label: "WikiTree — brother Nathaniel B Knight (Knight-12905)"
    url: "https://www.wikitree.com/wiki/Knight-12905"
media: []
---
## Summary

Named sibling of **Nathaniel B. Knight** (same parents John × Lydia Roberts). Soft-ingest **Probable** / `verified: false` — pack **C834**. WikiTree Knight-5642: b. **28 Jun 1764** Somersworth NH; brother of Nathaniel; husband Patience Garey parked (no climb). Death blank. Do not confuse with grandfather `daniel_knight_sr` (Knight-6274) or nephew lines. Do not climb.

## Assertions

| Claim | Confidence | Source | Source URL | Notes |
| --- | --- | --- | --- | --- |
| Child of John Knight × Lydia Roberts; sibling of Nathaniel B. | Probable | WT Daniel + Nathaniel sibling links | https://www.wikitree.com/wiki/Knight-5642 | Soft sibling **C834** |

## Sources

- [WikiTree — Daniel Knight (Knight-5642)](https://www.wikitree.com/wiki/Knight-5642)
- [WikiTree — Nathaniel B Knight (Knight-12905)](https://www.wikitree.com/wiki/Knight-12905)

## Open questions

- Death / burial / FAG
- Spouse Patience Garey — parked
---
