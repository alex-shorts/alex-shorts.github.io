# Bible worksheets

Weekly biblical imagery studies for family. Open a theme, print student or teacher sheets from the browser.

## Catalog

See [index.html](index.html) — driven by `worksheets.json`.

## Themes

| Theme | Student | Teacher |
| --- | --- | --- |
| Shield (new) | [student](shield/student.html) | [PDF](shield/teacher-guide.pdf) |
| Tent (new) | [student](tent/student.html) | [PDF](tent/teacher-guide.pdf) |
| Angel of the Lord (in progress) | — | [image options](angel-of-the-lord/image-options.html) |
| Dragon (new) | [student](dragon/student.html) · [versions](dragon/versions/index.html) | [PDF](dragon/teacher-guide.pdf) |
| Bread | [student](bread/student.html) | [teacher](bread/teacher.html) |
| Lamb | [student](lamb/student.html) | [teacher](lamb/teacher.html) |
| Rock | [student](rock/student.html) · [versions](rock/versions/index.html) · [after this week](rock/family-recap.html) | [teacher](rock/teacher.html) |
| Sword | [student](sword/student.html) | [teacher](sword/teacher.html) |
| Temple | [student](temple/student.html) | [teacher](temple/teacher.html) |

Sync from `biblical-imagery-library/worksheets/` when packs are approved.
