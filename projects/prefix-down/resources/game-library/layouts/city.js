...
export function paintRoad(scene, worldW, opts = {}) {
  ...
  if (props) {
    ...
    for (let x = 180; x < span; x += 720) {
      scene.add.image(x, H - 8, "sof-lamp").setOrigin(0.45, 1).setScale(S).setDepth(-1);
    }
    for (let x = 40; x < span; x += 860) {
      scene.add.image(x, H + 6, "sof-bush").setOrigin(0.5, 1).setScale(S).setDepth(-2);
    }
  }
